...
namespace raptor
{
	...
	public abstract class BinaryComponent : Component
	{
		...
		protected void Draw_Diamond_and_Text(System.Drawing.Graphics gr,
			int x, int diamond_top, string text,
			System.Drawing.Pen diamond_pen,
			bool draw_text)
		{
			System.Drawing.Pen line_pen;
			...
			if (this.Is_Wide_Diamond() && draw_text)
			{
				...
				gr.DrawRectangle(line_pen,
					x-W/4-this.drawing_text_width/2,
					diamond_top,H/4,H/4);
				if (this.is_compressed)
				{
					gr.DrawLine(line_pen,
						x-W/4-this.drawing_text_width/2+H/8,
						diamond_top,
						x-W/4-this.drawing_text_width/2+H/8,
						diamond_top+H/4);
				}
				gr.DrawLine(line_pen,
					x-W/4-this.drawing_text_width/2,
					diamond_top+H/8,
					x-W/4-this.drawing_text_width/2+H/4,
					diamond_top+H/8);

				// Draw the ^
				gr.DrawLine(diamond_pen,x,diamond_top,
					x+W/8,diamond_top+H/8);
				gr.DrawLine(diamond_pen,x,diamond_top,
					x-W/8,diamond_top+H/8);
				// Draw the bottom ^
				gr.DrawLine(diamond_pen,x,diamond_top+H,
					x+W/8,diamond_top+7*H/8);
				gr.DrawLine(diamond_pen,x,diamond_top+H,
					x-W/8,diamond_top+7*H/8);
				...
				// draw the >
				gr.DrawLine(diamond_pen,
					x+this.drawing_text_width/2+W/4,
					diamond_top+H/2,
					x+this.drawing_text_width/2,
			...
			else
			{
				// draw the + or - to expand
				gr.DrawRectangle(line_pen,
					x-W/2,diamond_top,H/4,H/4);
				if (this.is_compressed)
				{
					gr.DrawLine(line_pen,
						x-W/2+H/8,diamond_top,x-W/2+H/8,diamond_top+H/4);
				}
				gr.DrawLine(line_pen,
					x-W/2,diamond_top+H/8,x-W/2+H/4,diamond_top+H/8);

				gr.DrawLine(diamond_pen,x,diamond_top,
					x+W/2,diamond_top+H/2); // draw top right line
				gr.DrawLine(diamond_pen,x+W/2,
					diamond_top+H/2,x,diamond_top+H); // draw bottom right line
				gr.DrawLine(diamond_pen,x,diamond_top+H,
					x-W/2,diamond_top+H/2); // draw bottom left line
				gr.DrawLine(diamond_pen,x-W/2,diamond_top+H/2,
					x,diamond_top); // draw top left line
			}
			...
			if (draw_text) 
			{
				...
				if (this.Text.Length > 0)
				{
					...
					{
						rect = new System.Drawing.Rectangle(x-W/2, diamond_top+(H*7)/16, W-W/8,this.height_of_text);
					}
					...
					{
						gr.DrawString(this.getDrawText(), PensBrushes.default_times, PensBrushes.blackbrush, rect, PensBrushes.centered_stringFormat);
					}
				}
			}
		}
			...
			int buffer)
		{
			if (Component.full_text && draw_text)
			{
				...
				{
					sz = gr.MeasureString(
						this.getDrawText()+"XXXXX", PensBrushes.default_times, 
						width_of_text);
				...
				if (sz.Height > height_of_text*3/2 || width_of_text > 5*W/7) 
				{
		...
		public override bool check_expansion_click (int x, int y)
		{
			...
			if (this.first_child!=null && this.first_child.check_expansion_click(x,y))
			{
				return true;
			}
			if (this.second_child!=null && this.second_child.check_expansion_click(x,y))
			{
				return true;
			}
			if (this.Successor!=null)
