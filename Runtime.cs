...
namespace raptor
{
	...
	public class Runtime
	{
        ...
        public static void Add_Node_To_Heap(TreeNode node)
        {
            ...
            {
                if (node.GetHashCode() < (numbers_pkg.object_of((watchBox.Nodes[i] as Variable).
		...
		internal class Variable : TreeNode 
		{
			...
			public Variable(string name, numbers.value Value) : base() 
			{
				...
				this.Text = name + ": " + numbers_pkg.msstring_view_image(Value);
				Add_To_Updated(this);
			}
            ...
            public Variable(NClass.Core.ClassType ct)
            {
                ...
                Variable_Value = numbers_pkg.make_object_value((object) this);
                this.theClass = ct;
            ...
            private void addField(NClass.Core.Field f)
            {
                if (f.Type.Contains("[][]"))
                {
                    this.Nodes.Add(new Variable(f.Name, 1, 1, numbers_pkg.zero));
                }
                else if (f.Type.Contains("[]"))
                {
                    this.Nodes.Add(new Variable(f.Name, 1, numbers_pkg.zero));
                }
                else
                {
                    numbers.value val;
                    if (f.InitialValue != null && f.InitialValue != "")
                    {
                        ...
                        {
                            val = numbers_pkg.make_string_value(f.InitialValue);
                        }
                        else
                        {
                            val = numbers_pkg.make_number_value(f.InitialValue);
                        }
                    }
                    else
                    {
                        val = numbers_pkg.zero;
                    }
			...
			public Variable(string name, int index, numbers.value Value) : base() 
			{
				...
				this.Nodes.Add(new Variable("Size",numbers_pkg.make_value__3(index)));
                if (numbers_pkg.is_string(Value))
                {
                    for (int i = 1; i < index; i++)
                    {
                        this.Nodes.Add(new Variable("<" + i + ">", numbers_pkg.make_string_value("")));
                    }
                }
                else
                {
                    for (int i = 1; i < index; i++)
                    {
                        this.Nodes.Add(new Variable("<" + i + ">", numbers_pkg.make_value__2(0.0)));
                    }
                }
				...
				numbers.value Value) : base() 
			{
				...
				this.Nodes.Add(new Variable("Rows",numbers_pkg.make_value__3(index1)));
                if (numbers_pkg.is_string(Value))
                {
                    for (int i = 1; i < index1; i++)
                    {
                        this.Nodes.Add(new Variable("<" + i + ">", index2, numbers_pkg.make_string_value("")));
                    }
                }
                else
                {
                    for (int i = 1; i < index1; i++)
                    {
                        this.Nodes.Add(new Variable("<" + i + ">", index2, numbers_pkg.make_value__2(0.0)));
                    }
                }
			...
			public void set_value(numbers.value v)
			{
                if (!this.isConstant)
                {
                    ...
                    this.Text = this.Var_Name + ": " + numbers_pkg.msstring_view_image(v);
                    Add_To_Updated(this);
                }
                else
                {
			...
			public void Extend_1D(int index)
			{
				...
				int count = numbers_pkg.integer_of (((Variable) 
					(this.FirstNode)).Variable_Value);
				// do we need to extend?
				if (index > count)
				{
					((Variable) this.FirstNode).Variable_Value =
						numbers_pkg.make_value__3(index);
            ...
            public void set_field(string name, numbers.value f)
            {
                ...
                field.Text = name + ": " + numbers_pkg.msstring_view_image(f);
			...
			public void set_1D_value(int index, numbers.value f)
			{
				...
				if (this.Kind==Variable_Kind.Value && numbers_pkg.is_string(this.Variable_Value))
				{
					int c;
                    if (numbers_pkg.is_number(f))
                    {
                        c = (int)numbers_pkg.integer_of(f);
                    }
                    else if (numbers_pkg.is_character(f))
                    {
                        c = (int)f.c;
                    }
                    else if (numbers_pkg.is_string(f) && numbers_pkg.length_of(f) == 1)
                    {
                        c = (int)f.s[0];
                    }
                    else
					...
					// pad with spaces
					string new_string;
					if (index > numbers_pkg.length_of(this.Variable_Value))
					{
						new_string = this.Variable_Value.s +
							new String(' ',index-numbers_pkg.length_of(this.Variable_Value)-1) + (char) c;
					}
					else
					{
					...
					this.Variable_Value = numbers_pkg.make_string_value(new_string);
				...
				else if (this.Kind==Variable_Kind.One_D_Array) 
				{
                    ...
                    this.Nodes[index].Text = "<" + index + ">: " + numbers_pkg.msstring_view_image(f);
			...
			public void set_2D_value(int index1, int index2, numbers.value Value)
			{
				...
				// how many entries are there now
				int row_count = numbers_pkg.integer_of(((Variable) 
					(this.FirstNode)).Variable_Value);
				int col_count = numbers_pkg.integer_of(((Variable) 
				...
				this.Nodes[index1].Nodes[index2].Text = 
					"<"+index2+">: " + numbers_pkg.msstring_view_image(Value);
			...
			public numbers.value getArrayElement(int index)
			{
				...
				{
					if (numbers_pkg.integer_of(((Variable) 
						(this.FirstNode)).Variable_Value) >=
						index)
					{
				...
				else if (this.Kind==Variable_Kind.Value && numbers_pkg.is_string(this.Variable_Value)) 
				{
					if (numbers_pkg.length_of (this.Variable_Value) >=
						index)
					{
						return numbers_pkg.make_value__4 (this.Variable_Value.s[index-1]);
					}
					else
					{
						throw new Exception(this.Var_Name + " doesn't have " +
			...
			public int getArraySize()
			{
				if (this.Kind==Variable_Kind.One_D_Array) 
				{
					...
					return numbers_pkg.integer_of(((Variable) 
						(this.FirstNode)).Variable_Value);
				}
				else if (this.Kind==Variable_Kind.Value && numbers_pkg.is_string(this.Variable_Value))
				{
					return numbers_pkg.length_of(this.Variable_Value);
				}
				else
				{
					throw new Exception(this.Var_Name + " is not a 1D array.");
			...
			public void Overwrite(Variable source) 
			{
				...
				switch (this.Kind) 
				{
					...
					case Variable_Kind.Value:
                        try
                        {
                            if (!numbers_pkg.Oeq(this.Variable_Value, source.Variable_Value))
                            {
		...
		private static System.Windows.Forms.TreeView watchBox;
		public Runtime()
		{
		...
		public static void Init(Visual_Flow_Form p, MasterConsole MC,
			System.Windows.Forms.TreeView watch) 
		{
			parent = p;
			console = MC;
			watchBox = watch;
		...
		private static numbers.value getVariable_Delegate(string s)
		{
			...
			if (temp!=null)
			{
                ...
                {
                    return numbers_pkg.make_1d_ref(temp);
                }
                else if (temp.Kind == Variable_Kind.Two_D_Array && processing_parameter_list)
                {
                    return numbers_pkg.make_2d_ref(temp);
                }
            }
        ...
        public static Object getVariableContext(string s)
        {
            Object[] parameters = new Object[1];
            parameters[0] = s;
            return numbers_pkg.object_of((numbers.value)watchBox.Invoke(getVariable_delegate, parameters));
        }
		//****************************************************
		// getVariable
		// Returns the current value of an existing variable
		...
		public static double[] getArray(string s)
		{
			...
			if (temp!=null)
			{
				if (temp.Kind==Variable_Kind.One_D_Array) 
				{
					...
					for (int i=0; i<count; i++) 
					{
						result[i] = numbers_pkg.long_float_of(temp.getArrayElement(i+1));
					}
					return result;
				}
				else
		...
		public static int[] getIntArray(string s)
		{
			...
			if (temp!=null)
			{
				if (temp.Kind==Variable_Kind.One_D_Array) 
				{
					...
					for (int i=0; i<count; i++) 
					{
						result[i] = (int) numbers_pkg.long_float_of(temp.getArrayElement(i+1));
					}
					return result;
				}
				else
        ...
        public static object getVariableContext1D(string s, int index)
        {
            ...
            if (temp != null)
            {
                return numbers_pkg.object_of(temp.getArrayElement(index));
            }
            throw new Exception(s + " not found.");
        }
            ...
            int index1, int index2)
        {
            ...
            if (temp != null)
            {
                return numbers_pkg.object_of(temp.get2DArrayElement(index1, index2));
            }
            throw new Exception(s + " not found.");
        }
			...
			if (temp!=null)
			{
				if (temp.Kind==Variable_Kind.Two_D_Array) 
				{
					...
					for (int i=0; i<row_count; i++) 
					{
						...
						{
							result[i][j] = numbers_pkg.long_float_of(temp.get2DArrayElement(i+1,j+1));
						}
					}
					return result;
				}
			...
			if (temp!=null)
			{
				if (temp.Kind==Variable_Kind.Two_D_Array) 
				{
					...
					for (int i=0; i<row_count; i++) 
					{
						...
						for (int j=0; j<col_count; j++) 
						{
							result[i][j] = numbers_pkg.integer_of(temp.get2DArrayElement(i+1,j+1));
						}
					}
					return result;
				}
        ...
        public static bool is_Character(string s)
        {
            ...
            {
                return (temp.Kind == Variable_Kind.Value) &&
                    numbers_pkg.is_character(temp.Variable_Value);
            }
            else
            {
                return false;
        ...
        public static bool is_String(string s)
		{
            ...
            {
                return (temp.Kind == Variable_Kind.Value) &&
                    numbers_pkg.is_string(temp.Variable_Value);
            }
            else
            {
                return false;
        ...
        public static bool is_Scalar(string s)
        {
            ...
            {
                return (temp.Kind == Variable_Kind.Value) &&
                    numbers_pkg.is_number(temp.Variable_Value);
            }
            else
            {
                return false;
        ...
        public static void Redirect_Input(int yes_or_no)
		{
			if (raptor_files_pkg.network_is_redirected ||
				Visual_Flow_Form.command_line_input_redirect)
			{
				return;
			}
			if (yes_or_no==0)
		...
		public static void Redirect_Input(string filename)
		{
            ...
            //MessageBox.Show("redirecting to : " + filename + System.IO.File.Exists(filename));
